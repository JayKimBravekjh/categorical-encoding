...
class HotDeckEncoder(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, verbose=0, cols=None, drop_invariant=False, return_df=True, handle_missing='value',
                     handle_unknown='value', min_samples_leaf=1, smoothing=1.0, n_draws=10, random_state=5):
    ...
    def fit(self, X, y, **kwargs):
        ...
        self.hotdeck_mapping, self.smoove_mapping = self.fit_target_encoding(X_ordinal, y)
    ...
    def fit_target_encoding(self, X, y):
        ...
        for switch in self.ordinal_encoder.category_mapping:
            ...
            stats = y.groupby(X[col]).agg(['count'])
            smoove = 1 / (1 + np.exp(-(stats['count'] - self.min_samples_leaf) / self.smoothing))

            ys = y.groupby(X[col]).apply(list)


            hotdeck_mapping[col] =ys
            smoove_mapping[col] = smoove

        return hotdeck_mapping, smoove_mapping
    ...
    def _sample_single(self, values, smoove):
        if np.isnan(smoove) or values is np.nan:    #TODO: this should not be happening
            smoove = 0 
            cat_sample = 0
        else:
            cat_sample = np.random.choice(values)
    ...
    def target_encode_single(self, X_in):
        ...
        for col in self.cols:
            observation_values = X[col].map(self.hotdeck_mapping[col])
            observation_smoove = X[col].map(self.smoove_mapping[col])

            normal_vectorized = np.vectorize(self._sample_single)
            X[col] = pd.Series(normal_vectorized(values=observation_values, smoove=observation_smoove))
