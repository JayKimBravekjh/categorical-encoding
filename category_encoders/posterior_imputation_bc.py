...
class PosteriorImputationEncoderBC(BaseEstimator, TransformerMixin):
    ...
    def _compute_posterior_parameters(self, successes, failures, prior_values : Tuple[float, float] = (0,0), prior_scaling_factor=1.0):
        return (prior_values[0] * prior_scaling_factor + successes, prior_values[1] * prior_scaling_factor + failures)
    ...
    def _train(self, X, y):
        ...
        prior = self._compute_posterior_parameters(y.sum(), y.shape[0]-y.sum())
    ...
    def _sample_single(self, successes, failures):
        x = np.random.beta(successes+1, failures+1)
        return x
    ...
    def _score_one_draw(self, X_in):
        ...
        for col in self.cols:
            # Score the column
            successes = self.mapping[col][0]
            failures = self.mapping[col][1]
            sample_result = pd.Series(sample_function(successes, failures), index=successes.index)
