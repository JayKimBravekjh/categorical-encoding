...
class ProbabilisticTargetEncoderForBinaryClassification(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, verbose=0, cols=None, drop_invariant=False, return_df=True, handle_missing='value',
                     handle_unknown='value', min_samples_leaf=1, smoothing=1.0, n_draws=20, random_state=5):
    ...
    def transform(self, X, y=None, override_return_df=False, is_train = False):
        ...
        X = self.target_encode(X, is_train)
    ...
    def fit_transform(self, X, y=None, **fit_params):
        ...
        return self.fit(X, y, **fit_params).transform(X, y, is_train=True)

    def _sample_single(self, pi):
        return np.random.binomial(n=1, p=pi, size=1)
    ...
    def target_encode_single(self, X_in):
        ...
        for col in self.cols:
            observation_mean = X[col].map(self.mapping[col])

            bernoulli_vectorized = np.vectorize(self._sample_single)
            X[col] = pd.Series(bernoulli_vectorized(pi=observation_mean))

        return X


    def target_encode(self, X_in, is_train=False):

        if is_train:
            return pd.concat([self.target_encode_single(X_in) for _ in range(self.n_draws)])
    ...
    def _get_bits(self, nbr, size):
        return [nbr >> i & 1 for i in range(size)]

    
    def predict(self, X, model_predictor):
        '''
        We get all permutations of all categorical feature values, run predict on the model and
        weight them based on the product of the category probabilities
        '''
        preds = np.zeros(shape=(X.shape[0]))
        n_combinations = 2 ** len(self.cols)    #OK, there could be a lot of these... Might have performance problems
        for combination in range(n_combinations):
            X_copy = X.copy()
            bits = self._get_bits(combination, len(self.cols))
            prob = np.ones(shape=(X.shape[0]))
            for col_index, col in enumerate(self.cols):
                col_value = bits[col_index]
                prob *= X[col] if col_value == 1 else (1-X[col])
                X_copy[col] = col_value
                
            preds += prob * model_predictor(X_copy)
        return preds
